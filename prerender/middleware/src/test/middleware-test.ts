...
function makeApp(options: rendertron.Options) {
  return express()
    .use(rendertron.***REMOVED***(options))
...
function makeProxy() {
  return express().use((req, res) => {
    res.end('proxy ' + ***REMOVED***(req.url.substring(1)));
  });
}
...
test('makes a middleware function', async (t) => {
  const m = rendertron.***REMOVED***({ proxyUrl: 'http://example.com' });
  t.truthy(m);
});

test('throws if no proxyUrl given', async (t) => {
  const makeMiddlewareUntyped = rendertron.***REMOVED*** as (
    options?: unknown
  ) => express.Application;
...
test('adds shady dom parameter', async (t) => {
  const proxyUrl = await listen(makeProxy());
  const appUrl = await listen(makeApp({ proxyUrl, ***REMOVED***: true }));
...
test('respects custom user agent pattern', async (t) => {
  const proxyUrl = await listen(makeProxy());
  const appUrl = await listen(makeApp({ proxyUrl, ***REMOVED***: /borg/ }));
...
test('respects custom exclude url pattern', async (t) => {
  const proxyUrl = await listen(makeProxy());
  const appUrl = await listen(makeApp({ proxyUrl, ***REMOVED***: /foo/ }));
