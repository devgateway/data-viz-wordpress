 ...
 */
export function ***REMOVED***(options: Options): express.Handler {
  ...
  const ***REMOVED*** =
    options.***REMOVED*** || new RegExp(botUserAgents.join('|'), 'i');
  const ***REMOVED*** =
    options.***REMOVED*** ||
    new RegExp(`\\.(${***REMOVED***.join('|')})$`, 'i');
  ...
  const ***REMOVED*** = allowedForwardedHosts.length
    ? options.***REMOVED*** || 'X-Forwarded-Host'
    : null;

  return function ***REMOVED***(req, res, next) {
    const ua = req.headers['user-agent'];
    if (
      ua === undefined ||
      !***REMOVED***.test(ua) ||
      ***REMOVED***.test(req.path)
    ) {
      next();
      return;
    }
    const forwardedHost = ***REMOVED*** && req.get(***REMOVED***);
    const host =
      forwardedHost && allowedForwardedHosts.includes(forwardedHost)
        ? forwardedHost
        : req.get('host');
    const incomingUrl = req.protocol + '://' + host + req.originalUrl;
    let renderUrl = proxyUrl + ***REMOVED***(incomingUrl);
    if (***REMOVED***) {
