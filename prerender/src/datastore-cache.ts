...
export class ***REMOVED*** {
  ...
  async clearCache() {
    ...
    const entityKeys = entities.map(
      (entity: Record<string, unknown>) =>
        (entity as ***REMOVED***)[Datastore.KEY]
    );
  ...
  async cacheContent(
    ...
    if (parseInt(this.config.cacheConfig.***REMOVED***) !== -1) {
      ...
      this.datastore.runQuery(query, function (err, entities) {
        ...
        const ***REMOVED*** = (entities || []).map(
          (entity: Record<string, unknown>) =>
            (entity as ***REMOVED***)[Datastore.KEY]
        );
        if (
          ***REMOVED***.length >=
          parseInt(self.config.cacheConfig.***REMOVED***)
        ) {
          const toRemove =
            ***REMOVED***.length -
            parseInt(self.config.cacheConfig.***REMOVED***) +
            1;
          const toDelete = ***REMOVED***.slice(0, toRemove);
    ...
    const entity = {
      ...
      data: [
        { name: 'saved', value: now },
        {
          ...
          value: new Date(
            now.getTime() +
              parseInt(this.config.cacheConfig.***REMOVED***) * 60 * 1000
          ),
        },
        {
          name: 'headers',
  ...
  async ***REMOVED***(ctx: Koa.Context, key: entity.Key) {
  ...
  middleware() {
    ...
    return async function (
      ...
      const results = await this.***REMOVED***(ctx, key);
