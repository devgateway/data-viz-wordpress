...
test('health check responds correctly', async (t: ***REMOVED***) => {
  const res = await server.get('/_ah/health');
  t.is(res.status, 200);
});

test('renders basic script', async (t: ***REMOVED***) => {
...
test('renders script after page load event', async (t: ***REMOVED***) => {
  const res = await server.get(`/render/${testBase}script-after-load.html`);
  t.is(res.status, 200);
  t.true(res.text.indexOf('***REMOVED***') !== -1);
});

test('renders HTML docType declaration', async (t: ***REMOVED***) => {
  const res = await server.get(`/render/${testBase}include-doctype.html`);
  t.is(res.status, 200);
  t.true(res.text.indexOf('<!DOCTYPE html>') !== -1);
});

test('sets the correct base URL for a subfolder', async (t: ***REMOVED***) => {
...
test('sets the correct base URL for the root folder', async (t: ***REMOVED***) => {
  const res = await server.get(`/render/${testBase}basic-script.html`);
  const matches = res.text.match('<base href="([^"]+)">');
  const baseUrl = matches ? matches[1] : '';
...
test('sets the correct base URL for an already defined base as /', async (t: ***REMOVED***) => {
...
test('sets the correct base URL for an already defined base as directory', async (t: ***REMOVED***) => {
...
test('renders shadow DOM - polyfill loader', async (t: ***REMOVED***) => {
  const res = await server.get(
...
test('renders shadow DOM - polyfill loader - different flag', async (t: ***REMOVED***) => {
  const res = await server.get(
...
test('renders shadow DOM - webcomponents-lite.js polyfill', async (t: ***REMOVED***) => {
  const res = await server.get(
...
test('script tags and link[rel=import] tags are stripped', async (t: ***REMOVED***) => {
  const res = await server.get(
...
test('script tags for JSON-LD are not stripped', async (t: ***REMOVED***) => {
...
test('server status code should be forwarded', async (t: ***REMOVED***) => {
  const res = await server.get(`/render/${testBase}404`);
  t.is(res.status, 404);
});

test('http status code should be able to be set via a meta tag', async (t: ***REMOVED***) => {
...
test('http status codes need to be respected from top to bottom', async (t: ***REMOVED***) => {
...
test('screenshot is an image', async (t: ***REMOVED***) => {
...
test('screenshot accepts options', async (t: ***REMOVED***) => {
...
test('invalid url fails', async (t: ***REMOVED***) => {
  const res = await server.get(`/render/abc`);
  t.is(res.status, 403);
});

test('unknown url fails', async (t: ***REMOVED***) => {
  const res = await server.get(`/render/http://unknown.blah.com`);
  t.is(res.status, 400);
});

test('file url fails', async (t: ***REMOVED***) => {
  const res = await server.get(`/render/file:///dev/fd/0`);
  t.is(res.status, 403);
});

test('file url fails for screenshot', async (t: ***REMOVED***) => {
  const res = await server.get(`/screenshot/file:///dev/fd/0`);
  t.is(res.status, 403);
});

test('appengine internal url fails', async (t: ***REMOVED***) => {
  const res = await server.get(
    `/render/http://metadata.google.internal/***REMOVED***/v1beta1/instance/service-accounts/default/token`
  );
  t.is(res.status, 403);
});

test('appengine internal url fails for screenshot', async (t: ***REMOVED***) => {
  const res = await server.get(
    `/screenshot/http://metadata.google.internal/***REMOVED***/v1beta1/instance/service-accounts/default/token`
  );
  t.is(res.status, 403);
});
...
test('whitelist ensures other urls do not get rendered', async (t: ***REMOVED***) => {
  const mockConfig = {
...
test('unknown url fails safely on screenshot', async (t: ***REMOVED***) => {
  const res = await server.get(`/render/http://unknown.blah.com`);
  t.is(res.status, 400);
});

test('endpont for invalidating memory cache works if configured', async (t: ***REMOVED***) => {
  const mockConfig = {
    cache: 'memory' as const,
    cacheConfig: {
      ***REMOVED***: '120',
...
test('endpont for invalidating filesystem cache works if configured', async (t: ***REMOVED***) => {
  const mock_config = {
...
test('http header should be set via config', async (t: ***REMOVED***) => {
  const mock_config = {
    cache: 'memory' as const,
    cacheConfig: {
      ***REMOVED***: '120',
