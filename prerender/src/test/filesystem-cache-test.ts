...
import { ***REMOVED*** } from '../filesystem-cache';
...
const cache = new ***REMOVED***(config);
...
app.use(
  route.get('/', (ctx: Koa.Context) => {
    ***REMOVED***++;
    ctx.body = `Called ${***REMOVED***} times`;
  })
);

const ***REMOVED*** = function (timeout: number) {
  return new Promise((resolve) => {
    setTimeout(resolve, timeout);
  });
};

test('caches content and serves same content on cache hit', async (t: ***REMOVED***) => {
  const previousCount = ***REMOVED***;
...
test('caches headers', async (t: ***REMOVED***) => {
...
test('compression preserved', async (t: ***REMOVED***) => {
...
test('original status is preserved', async (t: ***REMOVED***) => {
...
test('cache entry can be removed', async (t: ***REMOVED***) => {
  let res = await server.get('/?***REMOVED***');
  t.is(res.status, 200);
  t.falsy(res.header['x-rendertron-cached']);
  t.false(new Date(res.header['x-rendertron-cached']) <= new Date());

  res = await server.get('/?***REMOVED***');
  t.is(res.status, 200);
  t.truthy(res.header['x-rendertron-cached']);
  t.true(new Date(res.header['x-rendertron-cached']) <= new Date());
  const key = cache.hashCode('/?***REMOVED***');
...
test('refreshCache refreshes cache', async (t: ***REMOVED***) => {
  let content = 'content';
  app.use(
