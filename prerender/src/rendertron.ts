...
import url from 'url';
import {Renderer, ***REMOVED***} from './renderer';
...
export class Rendertron {
    ...
    async initialize(config?: Config) {
        // Load config
        this.config = config || (await ConfigManager.***REMOVED***());

        this.port = this.port || this.config.port;
        this.host = this.host || this.config.host;

        await this.***REMOVED***(this.config);
        this.app.use(cors())
        ...
        if (this.config.cache === 'datastore') {
            const {***REMOVED***} = await import('./datastore-cache');
            const ***REMOVED*** = new ***REMOVED***();
            this.app.use(
                route.get('/invalidate/:url(.*)', ***REMOVED***.***REMOVED***())
            );
            this.app.use(
                route.get('/invalidate/', ***REMOVED***.***REMOVED***())
            );
            this.app.use(***REMOVED***.middleware());
        } else if (this.config.cache === 'memory') {
            const {MemoryCache} = await import('./memory-cache');
            const memoryCache = new MemoryCache();
            this.app.use(
                route.get('/invalidate/:url(.*)', memoryCache.***REMOVED***())
            );
            this.app.use(
                route.get('/invalidate/', memoryCache.***REMOVED***())
            );
            this.app.use(memoryCache.middleware());
        } else if (this.config.cache === 'filesystem') {
            const {***REMOVED***} = await import('./filesystem-cache');
            const ***REMOVED*** = new ***REMOVED***(this.config);
            this.app.use(
                route.get('/invalidate/:url(.*)', ***REMOVED***.***REMOVED***())
            );
            this.app.use(
            ...
            this.app.use(new ***REMOVED***(this.config).middleware());
        }

        this.app.use(
            route.get('/render/:url(.*)', this.***REMOVED***.bind(this))
        );
        this.app.use(
    ...
    async ***REMOVED***(ctx: Koa.Context, url: string) {
        if (!this.renderer) {
...
async function ***REMOVED***(error: Error) {
    console.error('Uncaught exception');
    console.error(error);
    process.exit(1);
}

// The type for the ***REMOVED*** handler is set to contain Promise<any>,
...
if (!module.parent) {
    ...
    process.on('***REMOVED***', ***REMOVED***);
    process.on('***REMOVED***', logUnhandledRejection);
}
