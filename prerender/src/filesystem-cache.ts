...
export class ***REMOVED*** {
  ...
  ***REMOVED***() {
  ...
  private ***REMOVED***(numCache: string[]) {
    const dirsDate = [];
  ...
  cacheContent(key: string, ctx: Koa.Context) {
    ...
    if (parseInt(this.config.cacheConfig.***REMOVED***) !== -1) {
      const numCache = fs.readdirSync(this.getDir(''));
      if (
        numCache.length >= parseInt(this.config.cacheConfig.***REMOVED***)
      ) {
        const toRemove =
          numCache.length -
          parseInt(this.config.cacheConfig.***REMOVED***) +
          1;
        let dirsDate = this.***REMOVED***(numCache);
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({ responseBody, ***REMOVED***, request })
    );
  }

  ***REMOVED***(ctx: Koa.Context, key: string): CacheContent | null {
    if (ctx.query.refreshCache) {
      ...
      try {
        ...
        const response = JSON.stringify(cacheFile.***REMOVED***);
        if (!payload) {
          return null;
        }
        ...
        const expires = new Date(
          saved.getTime() +
          parseInt(this.cacheConfig.***REMOVED***) * 60 * 1000
        );
        return {
          saved,
          expires,
  ...
  ***REMOVED***() {
  ...
  middleware() {
    ...
    return async function (
      ...
      // key is hashed crudely
      const key = this.hashCode(cacheKey);
      const content = await this.***REMOVED***(ctx, key);
      if (content) {
        // Serve cached content if its not expired.
        if (
          content.expires.getTime() >= new Date().getTime() ||
          parseInt(this.config.cacheConfig.***REMOVED***) === -1
        ) {
