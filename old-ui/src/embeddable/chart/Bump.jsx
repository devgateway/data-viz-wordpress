...
const Chart = ({
    ...
    const getColor = (id) => {
        if (filter.indexOf(id) > -1) {
            ...
            return ***REMOVED***(options.data.map(d => d.id).indexOf(id))
        }
    }


    const top = 5, right = 25, bottom = 60, left = 50;

    let margins = {top, right, bottom, left}

    if (***REMOVED*** == 'right' && showLegends) {

        margins = {top, right: margin, bottom, left}
    }
    if (***REMOVED*** == 'bottom' && showLegends) {

        margins = {top, right, bottom: margin, left}
    }
    if (***REMOVED*** == 'left' && showLegends) {

        margins = {top, right, bottom, left: margin}
    }

    const chartLegends = options.data.map(d => ({id: d.id, label: d.id, color: getColor(d.id)}))


    return (
        <div style={{height: height}}>

            {options && options.data && <***REMOVED***
            ...
            <div className={`legends container has-standard-12-font-size  ${***REMOVED***}`}>
               {showLegends && legendLabel &&
                ...
                {showLegends && chartLegends.map(legend => {
                    return (
                        <div className={"legend item"}>
                            <input type='checkbox' ***REMOVED*** onChange={() => toggle(legend.id)}/>
                            <label style={{***REMOVED***: legend.color, color: '#fff'}}>{legend.label}</label>
                        </div>)
                })}
            </div>
        </div>
