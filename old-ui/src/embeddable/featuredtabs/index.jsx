    ...
    >
      ...
      {!active ? (
        <Label onClick={onClick}>
          <Icon name="search" size="large" /> {moreLabel}
        </Label>
      ) : (
        <Label onClick={onClick}>
...
const FeaturedTabs = ({ posts, width, height, color, moreLabel }) => {
  ...
  return (
    ...
    >
      <Grid
        stackable
        columns={active != null ? 1 : posts.length}
        className="desktop"
      >
        {posts &&
          posts.map((post, i) => (
              ...
              >
                <Segment style={{ ***REMOVED***: arrayColors[i] }}>
                  {post.meta_fields && post.meta_fields.icon && (
                    <MediaProvider
                      id={post.meta_fields ? post.meta_fields.icon[0] : null}
                    >
                      <MediaConsumer>
                        <PostIcon />
                      </MediaConsumer>
                  ...
                  <PostTitle
                    as={"h2"}
                    post={post}
                    className={"has-standard-36-font-size has-white-color"}
                  />
                  <Label
                    className={"closeIcon"}
                ...
                <PostContent
                  ...
                  fluid={true}
                  post={post}
                  style={{ maxHeight: `calc(${height}px - 150px)` }}
                />
                <Label
                  className={"closeIconText"}
                  style={{ ***REMOVED***: `${arrayColors[i]}` }}
                  onClick={() => setActive(null)}
                >
                  <Icon name="times circle outline" size="large" /> Close{" "}
                </Label>
              </Grid.Column>
            </React.Fragment>
          ))}
      ...
      {posts.map((post, index) => {
        ...
        return (
          <React.Fragment key={post.id}>
            ...
            >
              ...
              >
                <div style={{ display: "flex", alignItems: "center" }}>
                  ...
                  <p
                    className="accordion-post-ft-title"
            ...
            <Accordion.Content
              className={"accordion-post-ft-content"}
              active={activeIndex === index}
            >
