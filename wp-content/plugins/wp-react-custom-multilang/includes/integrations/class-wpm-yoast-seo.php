...
class WPM_Yoast_Seo {
	...
	public function update_yoast_post_meta_tags()
	{
		...
		if ($table_exists) {
			...
			if($post_id){
				...
				//phpcs:ignore WordPress.DB.PreparedSQLPlaceholders.QuotedDynamicPlaceholderGeneration,WordPress.DB.PreparedSQL.NotPrepared, WordPress.DB.***REMOVED***.DirectQuery, WordPress.DB.***REMOVED***.NoCaching --Reason: Making use of built in functions fetched data of current language 
	...
	public function update_yoast_term_meta_tags($term_id, $tt_id, $taxonomy, $update, $args)
	{
		...
		// Check if yoast_indexable table exists
		//phpcs:ignore WordPress.DB.***REMOVED***.DirectQuery, WordPress.DB.***REMOVED***.NoCaching --Reason: As per requirement, We have to use the direct query.
		$table_exists = $wpdb->get_var( $wpdb->prepare("SHOW TABLES LIKE %s", $yoast_table_name) );
	...
	public function wpm_get_yoast_data( $object_id, $field_name, $type = '' )
	{
		...
		//phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.***REMOVED***.DirectQuery, WordPress.DB.***REMOVED***.NoCaching --Reason: As per requirement, We have to use the direct query.
