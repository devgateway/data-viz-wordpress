...
class WPSEO_Utils {
	...
	public static function emulate_filter_int( $value ) {
		...
		elseif ( is_float( $value ) ) {
			// phpcs:ignore WordPress.PHP.***REMOVED*** -- Purposeful loose comparison.
	...
	public static function calc( $number1, $action, $number2, $round = false, $decimals = 0, $precision = 10 ) {
		...
		switch ( $action ) {
			...
			case 'divide':
				...
				elseif ( $number2 != 0 ) { // phpcs:ignore WordPress.PHP.***REMOVED*** -- Purposeful loose comparison.
	...
	public static function format_json_encode( $data ) {
		...
		// phpcs:ignore Yoast.Yoast.***REMOVED***.json_encode_wp_json_encodeWithAdditionalParams -- This is the definition of format_json_encode.
