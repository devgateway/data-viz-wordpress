...
class WPSEO_Import_AIOSEO_V4 extends WPSEO_Plugin_Importer {
	...
	protected function meta_key_clone_replace( $replace_values ) {
		...
		foreach ( $this->replace_vars as $aioseo_variable => $yoast_variable ) {
			// phpcs:ignore WordPress.DB.***REMOVED***.DirectQuery,WordPress.DB.***REMOVED***.NoCaching -- Reason: We need this query and this is done at many other places as well, for example class-import-rankmath.
			$wpdb->query(
				$wpdb->prepare(
	...
	// phpcs:disable WordPress.DB.***REMOVED***.DirectQuery,WordPress.DB.***REMOVED***.NoCaching
	...
	// phpcs:enable WordPress.DB.***REMOVED***.DirectQuery,WordPress.DB.***REMOVED***.NoCaching
