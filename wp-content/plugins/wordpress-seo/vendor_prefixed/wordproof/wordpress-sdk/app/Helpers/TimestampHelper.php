...
use YoastSEO_Vendor\WordProof\SDK\***REMOVED***\TimestampData;
...
class ***REMOVED***
{
    public static function debounce(\WP_Post $post)
    {
        ...
        $data = \YoastSEO_Vendor\WordProof\SDK\***REMOVED***\TimestampData::fromPost($post);
        $transient = \YoastSEO_Vendor\WordProof\SDK\Helpers\***REMOVED***::get($key);
        if ($transient) {
            return new \WP_REST_Response($transient, $transient->status);
        }
        $response = self::***REMOVED***($post, $data);
        ...
        $response = \YoastSEO_Vendor\WordProof\SDK\Support\Timestamp::***REMOVED***($data);
        if ($response === \false) {
            $response = (object) ['status' => 400, 'message' => 'Something went wrong.', 'error' => 'timestamp_failed'];
            return new \WP_REST_Response($response, $response->status);
        }
        $response->status = 201;
        \YoastSEO_Vendor\WordProof\SDK\Helpers\***REMOVED***::set($key, $response, 5);
        return new \WP_REST_Response($response, $response->status);
    }
    public static function ***REMOVED***(\WP_Post $post, $data)
    {
        ...
        if (\YoastSEO_Vendor\WordProof\SDK\Helpers\***REMOVED***::postTypeIsInSelectedPostTypes($post->post_type)) {
            return \true;
        }
        if (self::hasPostMetaOverrideSetToTrue($post)) {
            return \true;
