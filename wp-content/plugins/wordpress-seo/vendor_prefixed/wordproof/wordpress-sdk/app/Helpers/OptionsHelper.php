...
class OptionsHelper
{
    ...
    public static function set($key, $value)
    {
        if (self::optionContainsOptions($key)) {
            $***REMOVED*** = self::secureOptionWithOptions($key, $value, 'sanitize');
            return \update_option(self::$prefix . $key, (object) $***REMOVED***);
        } else {
            $option = self::***REMOVED***($key);
            $***REMOVED*** = \YoastSEO_Vendor\WordProof\SDK\Helpers\***REMOVED***::sanitize($value, $option['escape']);
            return \update_option(self::$prefix . $key, $***REMOVED***);
        }
    }
    /**
     ...
     * @return bool
     */
    public static function ***REMOVED***($value)
    {
     ...
     */
    private static function ***REMOVED***($key)
    {
    ...
    private static function secureOptionWithOptions($key, $value, $method = 'sanitize')
    {
        ...
        if (\is_array($value)) {
            ...
            foreach ($value as $optionKey => $optionValue) {
                $optionConfig = self::***REMOVED***($key . '.options.' . $optionKey);
                if (!$optionConfig) {
                    continue;
                }
                if ($method === 'escape') {
                    ...
                    $securedValue = \YoastSEO_Vendor\WordProof\SDK\Helpers\***REMOVED***::sanitize($optionValue, $optionConfig['escape']);
                }
