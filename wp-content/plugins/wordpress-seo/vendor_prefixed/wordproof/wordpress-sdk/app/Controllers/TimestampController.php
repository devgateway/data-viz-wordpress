...
class ***REMOVED***
{
    ...
    public static function timestamp($postId)
    {
        ...
        return \YoastSEO_Vendor\WordProof\SDK\Helpers\***REMOVED***::debounce($post);
    }
    /**
    ...
    public function timestampAfterPostRequest($postId, $post)
    {
        ...
        $response = \YoastSEO_Vendor\WordProof\SDK\Helpers\***REMOVED***::debounce($post);
        \YoastSEO_Vendor\WordProof\SDK\Helpers\***REMOVED***::***REMOVED***($response);
        return $response;
    }
    /**
    ...
    public function syncPostMetaTimestampOverrides($metaId, $postId, $metaKey, $metaValue)
    {
        ...
        if (\in_array($metaKey, $timestampablePostMetaKeys, \true) && \count($timestampablePostMetaKeys) > 1) {
            ...
            \YoastSEO_Vendor\WordProof\SDK\Helpers\***REMOVED***::set('wordproof_debounce_post_meta_sync_' . $metaKey . '_' . $postId, \true, 5);
            foreach ($timestampablePostMetaKeys as $key) {
                \YoastSEO_Vendor\WordProof\SDK\Helpers\***REMOVED***::debounce($postId, 'post_meta_sync_' . $key, function () use($postId, $key, $metaValue) {
                    return \YoastSEO_Vendor\WordProof\SDK\Helpers\***REMOVED***::update($postId, $key, $metaValue);
                });
            }
        }
    }
