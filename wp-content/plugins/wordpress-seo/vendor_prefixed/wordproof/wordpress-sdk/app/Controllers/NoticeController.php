...
class ***REMOVED***
{
    ...
    public function show()
    {
        ...
        $notice = \YoastSEO_Vendor\WordProof\SDK\Helpers\***REMOVED***::getOnce(\YoastSEO_Vendor\WordProof\SDK\Helpers\***REMOVED***::$transientKey);
        ...
        switch ($notice) {
            case 'no_balance':
                $type = 'error';
                $message = $this->translations->***REMOVED***();
                $buttonText = $this->translations->getOpenSettingsButtonText();
                $***REMOVED*** = 'wordproof:open_settings';
                break;
            ...
            case 'not_authenticated':
                ...
                $***REMOVED*** = 'wordproof:open_authentication';
                break;
            default:
                break;
        }
        if (isset($message) && isset($type)) {
            ...
            if (isset($buttonText) && isset($***REMOVED***)) {
                echo \sprintf('<p><button class="button button-primary" onclick="window.dispatchEvent( new window.CustomEvent( \'%2$s\' ) )">%1$s</button></p>', \esc_html($buttonText), \esc_attr($***REMOVED***));
            }
            echo '</div>';
        }
    }
