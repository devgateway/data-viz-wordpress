...
final class Middleware
{
    ...
    public static function httpErrors()
    {
        return function (callable $handler) {
            return function ($request, array $options) use($handler) {
                ...
                return $handler($request, $options)->then(function (\YoastSEO_Vendor\Psr\Http\Message\***REMOVED*** $response) use($request) {
                    $code = $response->getStatusCode();
                    if ($code < 400) {
                        return $response;
                    }
                    throw \YoastSEO_Vendor\GuzzleHttp\Exception\***REMOVED***::create($request, $response);
                });
            };
        };
    }
     ...
     * @param ***REMOVED*** $formatter Formatter used to create message strings.
     * @param string           $logLevel Level at which to log requests.
     *
     * @return callable Returns a function that accepts the next handler.
     */
    public static function log(\YoastSEO_Vendor\Psr\Log\***REMOVED*** $logger, \YoastSEO_Vendor\GuzzleHttp\***REMOVED*** $formatter, $logLevel = 'info')
    {
