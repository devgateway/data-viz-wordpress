...
final class Utils
{
     ...
     * @return array
     */
    public static function ***REMOVED***($keys, array $data)
    {
        $result = [];
        foreach ($keys as &$key) {
            $key = \strtolower($key);
     ...
     */
    public static function copyToStream(\YoastSEO_Vendor\Psr\Http\Message\***REMOVED*** $source, \YoastSEO_Vendor\Psr\Http\Message\***REMOVED*** $dest, $maxLen = -1)
    {
        $bufferSize = 8192;
        if ($maxLen === -1) {
            while (!$source->eof()) {
     ...
     * @throws \***REMOVED*** on error.
     */
    public static function copyToString(\YoastSEO_Vendor\Psr\Http\Message\***REMOVED*** $stream, $maxLen = -1)
    {
        $buffer = '';
        if ($maxLen === -1) {
            while (!$stream->eof()) {
     ...
     * @throws \***REMOVED*** on error.
     */
    public static function hash(\YoastSEO_Vendor\Psr\Http\Message\***REMOVED*** $stream, $algo, $rawOutput = \false)
    {
        $pos = $stream->tell();
        if ($pos > 0) {
            $stream->rewind();
     ...
     * @return ***REMOVED***
     */
    public static function modifyRequest(\YoastSEO_Vendor\Psr\Http\Message\***REMOVED*** $request, array $changes)
    {
        if (!$changes) {
            return $request;
        }
        ...
        if ($request instanceof \YoastSEO_Vendor\Psr\Http\Message\ServerRequestInterface) {
            $new = (new \YoastSEO_Vendor\GuzzleHttp\Psr7\ServerRequest(isset($changes['method']) ? $changes['method'] : $request->getMethod(), $uri, $headers, isset($changes['body']) ? $changes['body'] : $request->getBody(), isset($changes['version']) ? $changes['version'] : $request->***REMOVED***(), $request->***REMOVED***()))->***REMOVED***($request->getParsedBody())->***REMOVED***($request->***REMOVED***())->***REMOVED***($request->***REMOVED***())->***REMOVED***($request->***REMOVED***());
            foreach ($request->getAttributes() as $key => $value) {
                $new = $new->withAttribute($key, $value);
            }
            return $new;
        }
        return new \YoastSEO_Vendor\GuzzleHttp\Psr7\Request(isset($changes['method']) ? $changes['method'] : $request->getMethod(), $uri, $headers, isset($changes['body']) ? $changes['body'] : $request->getBody(), isset($changes['version']) ? $changes['version'] : $request->***REMOVED***());
    }
    /**
     ...
     * @return string
     */
    public static function readLine(\YoastSEO_Vendor\Psr\Http\Message\***REMOVED*** $stream, $maxLength = null)
    {
        $buffer = '';
        $size = 0;
        while (!$stream->eof()) {
    ...
    public static function tryFopen($filename, $mode)
    {
        ...
        \set_error_handler(function () use($filename, $mode, &$ex) {
            $ex = new \***REMOVED***(\sprintf('Unable to open "%s" using mode "%s": %s', $filename, $mode, \func_get_args()[1]));
            return \true;
        });
        try {
