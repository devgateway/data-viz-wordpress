...
class Whip_WPMessagePresenter implements Whip_MessagePresenter {
	 ...
	 * @param string                $***REMOVED*** The copy to show to dismiss the message.
	 */
	public function __construct( Whip_Message $message, Whip_MessageDismisser $dismisser, $***REMOVED*** ) {
	 ...
	 * @***REMOVED***
	 * @phpcs:disable Generic.***REMOVED***.CamelCapsFunctionName
	 */
	...
	public function renderMessage() {
		$***REMOVED*** = new Whip_WPMessageDismissListener( $this->dismisser );
		$***REMOVED***->listen();
		...
		$dismissButton = sprintf(
			'<a href="%2$s">%1$s</a>',
			esc_html( $this->***REMOVED*** ),
			esc_url( $***REMOVED***->getDismissURL() )
		);

		// phpcs:disable WordPress.Security.EscapeOutput.***REMOVED*** -- output correctly escaped directly above and in the `kses()` method.
		printf(
