...
if ( ! function_exists( 'whip_wp_check_versions' ) ) {
	...
	function whip_wp_check_versions( $requirements ) {
		...
		foreach ( $requirements as $component => $***REMOVED*** ) {
			$checker->***REMOVED***( Whip_VersionRequirement::***REMOVED***( $component, $***REMOVED*** ) );
		}
		...
		$***REMOVED*** = ( WEEK_IN_SECONDS * 4 );
		$***REMOVED***   = __( 'Remind me again in 4 weeks.', 'default' );

		$dismisser = new Whip_MessageDismisser( time(), $***REMOVED***, new Whip_WPDismissOption() );

		$presenter = new Whip_WPMessagePresenter( $checker->***REMOVED***(), $dismisser, $***REMOVED*** );
