...
class Whip_RequirementsChecker {
	...
	public function __construct( $configuration = array(), $textdomain = 'default' ) {
		...
		$this->***REMOVED*** = new Whip_MessagesManager();
	 ...
	 */
	public function ***REMOVED***( Whip_Requirement $requirement ) {
	...
	private function requirementIsFulfilled( Whip_Requirement $requirement ) {
		$***REMOVED*** = $this->configuration->***REMOVED***( $requirement );
		$***REMOVED***  = $requirement->version();

		if ( in_array( $requirement->operator(), array( '=', '==', '===' ), true ) ) {
			return version_compare( $***REMOVED***, $***REMOVED***, '>=' );
		}

		return version_compare( $***REMOVED***, $***REMOVED***, $requirement->operator() );
	}
	...
	public function check() {
		foreach ( $this->requirements as $requirement ) {
			...
			$***REMOVED*** = $this->requirementIsFulfilled( $requirement );
	...
	private function addMissingRequirementMessage( Whip_Requirement $requirement ) {
		switch ( $requirement->component() ) {
			case 'php':
				$this->***REMOVED***->addMessage( new Whip_UpgradePhpMessage( $this->textdomain ) );
				break;
			default:
				$this->***REMOVED***->addMessage( new Whip_InvalidVersionRequirementMessage( $requirement, $this->configuration->***REMOVED***( $requirement ) ) );
				break;
		}
	}
	...
	public function hasMessages() {
		return $this->***REMOVED***->hasMessages();
	}
