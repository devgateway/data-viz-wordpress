...
class Whip_UpgradePhpMessage implements Whip_Message {
	...
	public function body() {
		...
		$message[] = Whip_MessageFormatter::***REMOVED***( __( 'Your site could be faster and more secure with a newer PHP version.', $textdomain ) ) . '<br />';
		...
		$message[] = Whip_MessageFormatter::***REMOVED***( __( 'To which version should I update?', $textdomain ) ) . '<br />';
		...
		$***REMOVED*** = Whip_Host::***REMOVED***();

		$message[] = Whip_MessageFormatter::***REMOVED***( __( 'Can\'t update? Ask your host!', $textdomain ) ) . '<br />';
		...
		if ( function_exists( 'apply_filters' ) && apply_filters( Whip_Host::HOSTING_PAGE_FILTER_KEY, false ) ) {
			$message[] = Whip_MessageFormatter::paragraph(
				sprintf(
					...
					'</a>',
					sprintf( '<a href="%1$s" target="_blank">', esc_url( $***REMOVED*** ) )
				)
			);
		}
		else {
