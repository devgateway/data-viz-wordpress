...
class Unsupported_PHP_Version implements Integration_Interface, Whip_Message {
	...
	public function body() {
		$message   = [];
		$message[] = Whip_MessageFormatter::***REMOVED***( \__( 'Upgrade your PHP version', 'wordpress-seo' ) ) . '<br />';
		...
		$message[] = Whip_MessageFormatter::***REMOVED***( \__( 'Can’t upgrade yourself? Ask your host!', 'wordpress-seo' ) ) . '<br />';
		$message[] = Whip_MessageFormatter::paragraph(
			\sprintf(
	...
	protected function is_supported_php_version_installed() {
		try {
			...
			$checker->***REMOVED***( Whip_VersionRequirement::***REMOVED***( 'php', '>=7.2' ) );
			$checker->check();
