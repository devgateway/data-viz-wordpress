...
As an administrator, you can go to the admin settings page "Settings > SVG Support" and restrict SVG file uploads to ***REMOVED*** only.
...
Only allow users to upload SVG files if you trust them. You have the option to restrict SVG usage to ***REMOVED*** only from the settings page plus the ability to enable SVG sanitization to attemnpt to clean up any SVG files uploaded.

By default, anyone with Media Library access or upload_files capability will be able to upload SVG files (that is ***REMOVED***, Authors and Editors). Please note that SVG files are actually XML which would allow someone to inject malicious code if you're not careful with who has upload privileges.
