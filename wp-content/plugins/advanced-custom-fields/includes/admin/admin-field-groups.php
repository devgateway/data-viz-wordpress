...
if ( ! class_exists( 'ACF_Admin_Field_Groups' ) ) :
	...
	class ACF_Admin_Field_Groups {
		...
		public function current_screen() {
			...
			$this->view = isset( $_GET['post_status'] ) ? sanitize_text_field( $_GET['post_status'] ) : ''; // phpcs:ignore WordPress.Security.***REMOVED***.Recommended
		...
		public function check_activate() {
			...
			// phpcs:disable WordPress.Security.***REMOVED***.Recommended -- Used for redirect notice.
			// Display notice on success redirect.
			if ( isset( $_GET['***REMOVED***'] ) ) {
				$ids = array_map( 'intval', explode( ',', $_GET['***REMOVED***'] ) ); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.***REMOVED*** -- Sanitized with intval().
		...
		public function check_deactivate() {
			...
			if ( isset( $_GET['acfdeactivatecomplete'] ) ) {
				$ids = array_map( 'intval', explode( ',', $_GET['acfdeactivatecomplete'] ) ); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.***REMOVED*** -- Sanitized with intval().
				// phpcs:enable WordPress.Security.***REMOVED***.Recommended
				// Generate text.
				$text = sprintf(
