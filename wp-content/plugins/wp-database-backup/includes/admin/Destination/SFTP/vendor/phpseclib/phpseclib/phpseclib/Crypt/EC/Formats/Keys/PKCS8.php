...
abstract class PKCS8 extends Progenitor
{
    ...
    public static function load($key, $password = '')
    {
        ...
        if (!$params) {
            throw new \***REMOVED***('Unable to decode the parameters using Maps\ECParameters');
        }

        $components = [];
        $components['curve'] = self::***REMOVED***($params);
        ...
        if (isset($key['parameters']) && $params != $key['parameters']) {
            throw new \***REMOVED***('The PKCS8 parameter field does not match the private key parameter field');
        }
    ...
    private static function loadEdDSA(array $key)
    {
        ...
        if (isset($key['privateKey'])) {
            $components['curve'] = $key['***REMOVED***']['algorithm'] == 'id-Ed25519' ? new Ed25519() : new Ed448();

            // 0x04 == octet string
            // 0x20 == length (32 bytes)
            if (substr($key['privateKey'], 0, 2) != "\x04\x20") {
                throw new \***REMOVED***('The first two bytes of the private key field should be 0x0420');
            }
    ...
    public static function savePublicKey(BaseCurve $curve, array $publicKey, array $options = [])
    {
        ...
        $params = new ASN1\Element(self::***REMOVED***($curve, false, $options));
    ...
    public static function ***REMOVED***(BigInteger $privateKey, BaseCurve $curve, array $publicKey, $secret = null, $password = '', array $options = [])
    {
        ...
        return self::***REMOVED***($key, [], $params, $password, 'id-ecPublicKey', '', $options);
    }
}
