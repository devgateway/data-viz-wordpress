...
class WP_Menu_Image
{
    ...
    )
    {
        ...
        if ( isset( $item->thumbnail_id ) && '' !== $item->thumbnail_id && $item->thumbnail_id > 0 ) {
            ...
            if ( !empty($args->walker) && class_exists( '***REMOVED***' ) && $args->walker instanceof ***REMOVED*** && !is_null( $depth ) && $depth === 0 ) {
                $class .= ' nav-top-link';
            }
        ...
        if ( isset( $item->menu_image_icon_type ) ) {
            ...
            if ( $item->menu_image_icon_type != 'icon' ) {
                ...
                if ( isset( $item->thumbnail_id ) && '' !== $item->thumbnail_id && $item->thumbnail_id > 0 ) {
                    ...
                    $position = ( $item->title_position ? $item->title_position : apply_filters( 'menu_image_default_title_position', 'after' ) );
                    $class = "menu-image-title-{$position}";
                    $this->***REMOVED***( $image_size, $item->thumbnail_id );
                    
                    if ( $item->thumbnail_hover_id ) {
                        $this->***REMOVED***( $image_size, $item->thumbnail_hover_id );
    ...
    {
        return $this->***REMOVED***( $data['attachment_id'], $data['size'] );
    }
     ...
     * @param int    $id
     */
    public function ***REMOVED***( $size, $id )
    {
     ...
     * @return bool
     */
    public function ***REMOVED***( $id, $size = null )
    {
    ...
    {
        if ( $this->***REMOVED***( $attachment->ID, $size ) ) {
