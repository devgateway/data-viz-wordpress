...
(function (factory) {
    ...
    var folderId = 0;
    var fID = 0;
    var ***REMOVED*** = folders_media_options.page_url;
    var ***REMOVED*** = "";
    var folderIDs = "";
    var ***REMOVED*** = false;
    ...
    var MediaLibraryOrganizerTaxonomyFilter = wp.media.view.***REMOVED***.extend({
    ...
    wp.media.view.***REMOVED*** = wp.media.view.***REMOVED***.extend({
        createToolbar: function() {
            ***REMOVED***.prototype.createToolbar.call(this);
    ...
    if (typeof wpMediaObj !== 'undefined' && typeof wpMediaObj.Uploader === 'function') {
        ...
        //                     $(".attachment-filters").val(***REMOVED***).trigger("change");
        //                 }
        //             }
        //         }
        //     },100);
            ...
            init: function () {
                if (this.uploader) {
                    ...
                    this.uploader.bind('BeforeUpload', function (uploader, file) {
                        ...
                        if(***REMOVED*** < filesInQueue) {
                            $(".media-folder-loader").show();
                            var progress_width = ***REMOVED***/filesInQueue*100;
                            $(".folder-meter").css("width", progress_width+"%");
                        }
                        ***REMOVED***++;
                        $("#current_upload_files").text(***REMOVED***);

                    });
                    this.uploader.bind('***REMOVED***', function (up, files) {
    ...
    $(document).ready(function(){
        ...
        ***REMOVED*** = folders_media_options.folder_settings;
        ...
        $(document).on("click", "a.jstree-anchor", function(e){
            ...
            $(".folder-modal #js-tree-menu").jstree('select_node', ***REMOVED***);
        ...
        $(document).on("submit", "#save-folder-form", function(e){
            ...
            ***REMOVED*** = $("#add-update-folder-name").val();
    ...
    function checkForUndoFunctionality() {
        if(folders_media_options.useFolderUndo == "yes") {
            ...
            }, parseInt(folders_media_options.***REMOVED***));
        }
    }
    ...
    function ***REMOVED***(folderSlug) {
        if(***REMOVED***.length > 0) {
            for(i=0; i<***REMOVED***.length; i++) {
                if(***REMOVED***[i]['slug'] == folderSlug) {
                    return ***REMOVED***[i]['folder_id'];
                }
            }
        }
        return "";
        ...
        $(".jstree-anchor:not(.ui-droppable)").droppable({
            ...
            drop: function( event, ui ) {
                ...
                if($(this).closest("ul.jstree-container-ul").find(".jstree-clicked").length) {
                    ...
                    ***REMOVED*** = $selectedNode.closest("li.jstree-node").attr('id');
                }
                if ( ui.draggable.hasClass( 'wcp-move-multiple')) {
                    if($(".wp-list-table input:checked").length) {
                        ...
                        $.ajax({
                            url: folders_media_options.ajax_url,
                            data: "post_ids=" + chkStr + "&type=" + folders_media_options.post_type + "&action=wcp_change_multiple_post_folder&folder_id=" + folderID+"&nonce="+nonce+"&status="+folders_media_options.taxonomy_status+"&taxonomy="+***REMOVED***,
                            method: 'post',
                            success: function (res) {
                                res = $.parseJSON(res);
                                if(res.status == "1") {
        ...
        $(treeId).jstree({
            ...
            ***REMOVED***();
            ...
            if(folders_media_options.default_folder != "" && folders_media_options.default_folder != 0) {
                var defaultFolder = ***REMOVED***(folders_media_options.default_folder);
                if(defaultFolder != "") {
    ...
    function ***REMOVED***(folderIDs) {
        ...
        if(!$(".folder-modal #js-tree-menu .jstree-anchor.jstree-clicked").length) {
            ...
            $.ajax({
                url: folders_media_options.ajax_url,
                data: "post_id=" + folderIDs + "&type=" + folders_media_options.post_type + "&action=wcp_remove_post_folder&folder_id=" + folderID + "&nonce=" + nonce + "&status=" + folders_media_options.taxonomy_status + "&taxonomy=" + ***REMOVED***,
                method: 'post',
                success: function (res) {
                    // window.location.reload();
                    ***REMOVED***();
            ...
            ***REMOVED*** = $(".folder-modal #js-tree-menu .jstree-anchor.jstree-clicked").closest(".jstree-node").attr("id");
            $.ajax({
                url: folders_media_options.ajax_url,
                data: {
                    post_id: folderIDs,
    ...
    function setCustomScrollForFolder() {
        ...
        if(scrollTop != 0) {
            jQuery("#custom-scroll-menu").***REMOVED***("scrollTo", scrollTop+"px",{scrollInertia:0});
        }*/
                    ...
                    success: function (res) {
                        ...
                        if (res.status == '1') {
                            ...
                            ***REMOVED***();
                            ...
                            if(!$(".folder-modal #wcp_folder_"+***REMOVED***).length) {
        ...
        if($("#media-attachment-taxonomy-filter").length) {
            ***REMOVED***(0);
            ...
            if($("#folder-posts-filter").length) {
                ...
                $("#wpbody").load(***REMOVED*** + " #wpbody-content", false, function (res) {
                    var obj = { Title: "", Url: ***REMOVED*** };
                    history.pushState(obj, obj.Title, obj.Url);
                    if (folders_media_options.show_in_page == "show" && !$(".tree-structure").length) {
                        $(".wp-header-end").before('<div class="tree-structure-content"><div class="tree-structure"><ul></ul><div class="clear clearfix"></div></div></div>');
    ...
    function ***REMOVED***(loadData) {
        ***REMOVED*** = $.ajax({
            url: folders_media_options.ajax_url,
            data: "type=" + folders_media_options.post_type + "&action=wcp_get_default_list&active_id="+***REMOVED***,
            method: 'post',
            beforeSend: function() {
                if(***REMOVED*** != null) {
                    ***REMOVED***.abort();
        ...
        if(folders_media_options.use_shortcuts == "yes") {
            ...
            $(document).keydown(function (e) {
                ...
                if(isCtrlPressed && (e.keyCode == 86 || e.which == 86)) {
                    /*if($(".folder-modal #js-tree-menu .jstree-anchor").is(":focus")) {
                        e.***REMOVED***();
                        ***REMOVED*** = $(".folder-modal #js-tree-menu .jstree-anchor:focus").closest("li.jstree-node").attr("id");
