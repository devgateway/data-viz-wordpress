(function (factory) {
    ...
    var folderIDs = "";
    var ***REMOVED*** = [];
    var ***REMOVED*** = wcp_settings.current_url;
    var foldersArray = [];
    var hasStars;
    var hasChildren;
    ...
    $(document).ready(function(){
        ...
        $(document).on("click", ".subsubsub a", function(e){
            if($("#js-tree-menu .jstree-anchor.jstree-clicked").length) {
                ...
                var folderSlug = ***REMOVED***(CurrentNode, 'slug');
                if(folderSlug != "") {
                    e.***REMOVED***();
        ...
        $(document).on("click", ".view-switch > a", function(e){
            if(location.search != "") {
                try {
                    ...
                    const searchArray = Object.fromEntries(new ***REMOVED***(str));
        ...
        ***REMOVED*** = parseInt(wcp_settings.selected_taxonomy);
        ...
        ***REMOVED*** = wcp_settings.folder_settings;
        ...
        var ***REMOVED*** = (wcp_settings.isRTL == "1" || wcp_settings.isRTL == 1)?"w":"e";
        ...
        $(document).on("click", "#js-tree-menu .folder-actions span.folder-inline-edit", function(e){
            ...
            if(e.pageX == undefined || e.pageY == undefined) {
                $(".dynamic-menu").css("left", (***REMOVED***));
                $(".dynamic-menu").css("top", (***REMOVED*** - 10));
                yPosition = ***REMOVED***;
            } else {
        ...
        $(document).on("click",".folder-color-option , .folder-color-default",function (e) {
            ...
            ***REMOVED***[folderPostId]['has_color'] = current_color;
            nonce = ***REMOVED***(folderID, 'nonce');
        ...
        $(document).on("click", ".sticky-folders .sticky-fldr > a", function(e) {
            ...
            if($(".jstree-node[id='"+folder_ID+"']").length) {
                ...
                $("#js-tree-menu").jstree('select_node', ***REMOVED***);
    ...
    function ***REMOVED***() {
        ...
        if(***REMOVED*** != "" && ***REMOVED*** != 0) {
            if($(".jstree-node[id='"+***REMOVED***+"']").length) {
                $("#js-tree-menu").jstree('select_node', ***REMOVED***);
                if($(".sticky-folders .sticky-folder-"+***REMOVED***+" a").length) {
                    $(".sticky-folders .sticky-folder-" + ***REMOVED*** + " a").addClass("active-item");
                }
            }
        }
        $(".ajax-preloader").hide();
    ...
    function ***REMOVED***(postId, filedName) {
        if(***REMOVED***.length > 0) {
            for(i=0; i<***REMOVED***.length; i++) {
                if(parseInt(***REMOVED***[i]['folder_id']) == parseInt(postId)) {
                    return ***REMOVED***[i][filedName];
                }
            }
        }
        return "";
    ...
    function ***REMOVED***() {
        ...
        if(folderIDs != "" && ($("#js-tree-menu a.jstree-clicked").length > 0 || ***REMOVED*** == "-1")) {
            if($("#media-attachment-taxonomy-filter").length) {
        ...
        if($("#media-attachment-taxonomy-filter").length) {
            ***REMOVED***(0);
            ...
            if($("#folder-posts-filter").length) {
                $("#folder-posts-filter").load(***REMOVED***+"&paged="+currentPage + " #posts-filter", function(){
                    var obj = { Title: "", Url: ***REMOVED***+"&paged="+currentPage };
                ...
                if(ajaxURL === false) {
                    ajaxURL = ***REMOVED***+"&paged="+currentPage;
                } else {
                    ajaxURL = ***REMOVED***;
                }
        ...
        if(***REMOVED***.length) {
            $("li.jstree-node").each(function(){
                ...
                if(folderPostId != null) {
                    if(***REMOVED***[folderPostId]['is_high'] == 1) {
                        $(this).addClass("is-high");
                    } else {
                        $(this).removeClass("is-high");
                    }
                    if(***REMOVED***[folderPostId]['is_sticky'] == 1) {
    ...
    function setCustomScrollForFolder() {
        ...
        /*var scrollTop = 0;
       if($("#custom-scroll-menu").hasClass("***REMOVED***")) {
           ...
           $("#custom-scroll-menu").***REMOVED***('destroy');

       }
       $("#custom-scroll-menu").***REMOVED***({
           axis:"y",
       ...
       if(scrollTop != 0) {
           $("#custom-scroll-menu").***REMOVED***("scrollTo", scrollTop+"px",{scrollInertia:0});
       }*/
    ...
    $(document).ready(function(){
        ...
        $(document).on("submit", "#save-folder-form", function(e){
            ...
            ***REMOVED*** = $.trim($("#add-update-folder-name").val());
    ...
    $(document).ready(function(){
        ...
        $(document).on("submit", "#update-folder-form", function(e){
            ...
            ***REMOVED*** = $("#update-folder-item-name").val();
            ...
            if($.trim(***REMOVED***) == "") {
                ...
                nonce = ***REMOVED***(fileFolderID, 'nonce');
                ...
                $.ajax({
                    ...
                    success: function (res) {
                        ...
                        if (result.status == '1') {
                            ...
                            if(folderPostId != null) {
                                ***REMOVED***[folderPostId]['nonce'] = result.nonce;
                                ***REMOVED***[folderPostId]['slug'] = result.slug;
                            }
                            add_menu_to_list();
                            $(".folder-popup-form").hide();
                            $(".folder-popup-form").removeClass("disabled");
            ...
            if(***REMOVED***) {
                ...
                $.ajax({
                    ...
                    success: function (res) {
                        res = $.parseJSON(res);
                        if(res.status == '1') {
                            var nextNode = ***REMOVED***(fileFolderID);
    ...
    $(document).ready(function(){
        $(document).on("click", "a.jstree-anchor", function(e) {
            ...
            $("active-item").removeClass("active-item");
            ***REMOVED*** = $(this).closest("li.jstree-node").attr("id");
            fileFolderID = $(this).closest("li.jstree-node").attr("id");
            $(".sticky-folders .sticky-folder-"+***REMOVED***+" a").addClass("active-item");
            $("#js-tree-menu .jstree-clicked").removeClass("jstree-clicked");
            $("#js-tree-menu").jstree('select_node', ***REMOVED***);
            $("#js-tree-menu #"+***REMOVED***+"_anchor").addClass("jstree-clicked");
            if(!$("#media-attachment-taxonomy-filter").length) {
                var folderSlug = ***REMOVED***(***REMOVED***, 'slug');
                ***REMOVED*** = wcp_settings.page_url + folderSlug+"&paged="+currentPage;
                $(".form-loader-count").css("width", "100%");
                if($("#folder-posts-filter").length) {
                    $("#folder-posts-filter").load(***REMOVED*** + " #posts-filter", function(){
                        var obj = { Title: folderSlug, Url: ***REMOVED*** };
                        history.pushState(obj, obj.Title, obj.Url);
                    ...
                    $("#wpbody").load(***REMOVED*** + " #wpbody-content", function(){
                        var obj = { Title: folderSlug, Url: ***REMOVED*** };
                        history.pushState(obj, obj.Title, obj.Url);
                        set_default_folders(folderSlug);
                        if (wcp_settings.show_in_page == "show" && !$(".tree-structure").length) {
                ...
                thisSlug = ***REMOVED***(thisIndex, 'slug');
                ***REMOVED*** = wcp_settings.page_url + thisSlug+"&paged="+currentPage;
        ...
        $(".header-posts").on("click", function(){
            ***REMOVED*** = "";
            ...
            if(!$("#media-attachment-taxonomy-filter").length) {
                currentPage = 1;
                ***REMOVED*** = wcp_settings.page_url+"&paged="+currentPage;
                $(".form-loader-count").css("width", "100%");
                if($("#folder-posts-filter").length) {
                    $("#folder-posts-filter").load(***REMOVED*** + " #posts-filter", function(){
        ...
        $(".un-categorised-items").on("click", function(){
            ***REMOVED*** = "-1";
            ...
            if(!$("#media-attachment-taxonomy-filter").length) {
                currentPage = 1;
                ***REMOVED*** = wcp_settings.page_url+"-1"+"&paged="+currentPage;
                $(".form-loader-count").css("width", "100%");
                if($("#folder-posts-filter").length) {
                    $("#folder-posts-filter").load(***REMOVED*** + " #posts-filter", function(){
                        var obj = { Title: "", Url: ***REMOVED*** };
    ...
    function ajaxAnimation() {
        ...
        $(".folder-loader-ajax svg#***REMOVED***").addClass("active").addClass("animated");
        setTimeout(function(){
            $(".folder-loader-ajax").removeClass("active");
            $(".folder-loader-ajax img").addClass("active");
            $(".folder-loader-ajax svg#***REMOVED***").removeClass("active").removeClass("animated");
        }, 2000);
    }
    ...
    function make_sticky_folder_menu() {
        ...
        $("#js-tree-menu li.jstree-node.is-sticky").each(function(){
            ...
            ***REMOVED*** += "<li data-folder-id='"+folder_ID+"' class='sticky-fldr "+hasStar+" sticky-folder-"+folder_ID+"'>" +
                "<a href='javascript:;'>" +
        ...
        $(".sticky-folders > ul").html(***REMOVED***);
    ...
    function ***REMOVED***() {
        ...
        if(***REMOVED*** != "") {
            $("#wcp_folder_"+***REMOVED***).addClass("active-item");
        }
            ...
            if($("#bulk-select").val() != "") {
                ...
                if($("#bulk-select").val() != "") {
                    if($("#bulk-select").val() == "-1") {
                        $.ajax({
                            url: wcp_settings.ajax_url,
                            data: "post_id=" + chkStr + "&type=" + wcp_settings.post_type + "&action=wcp_remove_post_folder&folder_id=" + $(this).val() + "&nonce=" + wcp_settings.nonce + "&status=" + wcp_settings.taxonomy_status + "&taxonomy=" + ***REMOVED***,
                            method: 'post',
                            success: function (res) {
                        ...
                        nonce = ***REMOVED***($("#bulk-select").val(), 'nonce');
                        $.ajax({
                            url: wcp_settings.ajax_url,
                            data: {
                                post_ids: chkStr,
    ...
    function ***REMOVED***(folderIDs) {
        var folderID = -1;
        if(***REMOVED*** == "" || ***REMOVED*** == 0) {
            nonce = wcp_settings.nonce;
            $.ajax({
                url: wcp_settings.ajax_url,
                data: "post_id=" + folderIDs + "&type=" + wcp_settings.post_type + "&action=wcp_remove_post_folder&folder_id=" + folderID + "&nonce=" + nonce + "&status=" + wcp_settings.taxonomy_status + "&taxonomy=" + ***REMOVED***,
                method: 'post',
                success: function (res) {
                    // window.location.reload();
                    ***REMOVED***();
            ...
            if(wcp_settings.post_type == "attachment") {
                if($(this).val() != "") {
                    ...
                    if ($(this).val() == "-1") {
                        $.ajax({
                            url: wcp_settings.ajax_url,
                            data: "post_id=" + checkStr + "&type=" + wcp_settings.post_type + "&action=wcp_remove_post_folder&folder_id=" + $(this).val() + "&nonce=" + wcp_settings.nonce + "&status=" + wcp_settings.taxonomy_status + "&taxonomy=" + ***REMOVED***,
                            method: 'post',
                            success: function (res) {
        ...
        if(wcp_settings.use_shortcuts == "yes") {
            ...
            $(document).keydown(function (e) {
                ...
                if(isCtrlPressed && (e.keyCode == 86 || e.which == 86)) {
                    /*if($("#js-tree-menu .jstree-anchor").is(":focus")) {
                        e.***REMOVED***();
                        ***REMOVED*** = $("#js-tree-menu .jstree-anchor:focus").closest("li.jstree-node").attr("id");
                        if(***REMOVED*** == "" || isNaN(***REMOVED***)) {
                            ***REMOVED*** = 0;
                        }
                        if(isFolderCopy != 0 && isFolderCopy != "" && isFolderCopy != ***REMOVED***) {
                            if(CPCAction == "cut") {
                                lastParentID = $("#" + isFolderCopy).data("parent");
                                ***REMOVED*** = isFolderCopy;
                                ***REMOVED*** = $("#" + isFolderCopy).index() + 1;
                                if(***REMOVED*** != "" && ***REMOVED*** != 0) {
                                    $('#js-tree-menu').jstree("move_node", "#" + isFolderCopy, "#" + ***REMOVED***, 0);
                                } else {
                                ...
                                copyFolders(isFolderCopy, ***REMOVED***);
                            }
                            ***REMOVED***();
                            CPCAction = "";
                            isFolderCopy = 0;
