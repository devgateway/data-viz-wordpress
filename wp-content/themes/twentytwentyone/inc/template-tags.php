...
if ( ! function_exists( 'twenty_twenty_one_posted_on' ) ) {
	...
	function twenty_twenty_one_posted_on() {
		...
		printf(
			...
			esc_html__( 'Published %s', '***REMOVED***' ),
...
if ( ! function_exists( 'twenty_twenty_one_entry_meta_footer' ) ) {
	...
	function twenty_twenty_one_entry_meta_footer() {
		...
		if ( ! is_single() ) {
			...
			if ( is_sticky() ) {
				echo '<p>' . esc_html_x( 'Featured post', 'Label for sticky posts', '***REMOVED***' ) . '</p>';
			}
			...
			if ( has_category() || has_tag() ) {
				...
				$categories_list = get_the_category_list( __( ', ', '***REMOVED***' ) );
				if ( $categories_list ) {
					printf(
						/* translators: %s: list of categories. */
						'<span class="cat-links">' . esc_html__( 'Categorized as %s', '***REMOVED***' ) . ' </span>',
						$categories_list // phpcs:ignore WordPress.Security.EscapeOutput
					);
				}

				/* translators: used between list items, there is a space after the comma. */
				$tags_list = get_the_tag_list( '', __( ', ', '***REMOVED***' ) );
				if ( $tags_list ) {
					printf(
						/* translators: %s: list of tags. */
						'<span class="tags-links">' . esc_html__( 'Tagged %s', '***REMOVED***' ) . '</span>',
	...
	function twenty_twenty_one_the_posts_navigation() {
		...
		the_posts_pagination(
			array(
				...
				'before_page_number' => esc_html__( 'Page ', '***REMOVED***' ),
				'mid_size'           => 0,
				'prev_text'          => sprintf(
