...
function twentynineteen_upgrade_notice() {
	...
	$message = sprintf( __( 'Twenty Nineteen requires at least WordPress version 4.7. You are running version %s. Please upgrade and try again.', '***REMOVED***' ), $GLOBALS['wp_version'] );
...
function twentynineteen_preview() {
	if ( isset( $_GET['preview'] ) ) {
		/* translators: %s: WordPress version. */
		wp_die( sprintf( __( 'Twenty Nineteen requires at least WordPress version 4.7. You are running version %s. Please upgrade and try again.', '***REMOVED***' ), $GLOBALS['wp_version'] ) );
	}
}
