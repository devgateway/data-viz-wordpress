...
function twentynineteen_customize_register( $wp_customize ) {
	...
	$wp_customize->add_control(
		...
		array(
			'type'     => 'radio',
			'label'    => __( 'Primary Color', '***REMOVED***' ),
			'choices'  => array(
				'default' => _x( 'Default', 'primary color', '***REMOVED***' ),
				'custom'  => _x( 'Custom', 'primary color', '***REMOVED***' ),
			),
			'section'  => 'colors',
			'priority' => 5,
		)
	...
	$wp_customize->add_control(
		new WP_Customize_Color_Control(
			...
			'primary_color_hue',
			array(
				'description' => __( 'Apply a custom color for buttons, links, featured images, etc.', '***REMOVED***' ),
				'section'     => 'colors',
				'mode'        => 'hue',
			)
		)
	...
	$wp_customize->add_control(
		'image_filter',
		array(
			'label'   => __( 'Apply a filter to featured images using the primary color', '***REMOVED***' ),
			'section' => 'colors',
			'type'    => 'checkbox',
		)
	);
...
function twentynineteen_customize_preview_js() {
	wp_enqueue_script( '***REMOVED***-customize-preview', get_theme_file_uri( '/js/customize-preview.js' ), array( 'customize-preview' ), '20181214', array( 'in_footer' => true ) );
}
...
function twentynineteen_panels_js() {
	wp_enqueue_script( '***REMOVED***-customize-controls', get_theme_file_uri( '/js/customize-controls.js' ), array(), '20181214', array( 'in_footer' => true ) );
}
