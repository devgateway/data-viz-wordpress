...
const CategoricalFilter = ({ value, index, items, onUpdateFilterValue }: CategoricalFilterProps) => {
    if (items) {
        const sortedItems = items ? items.sort(function (a, b) {
...
class BlockEdit extends BlockEditWithAPIMetadata<FilterProps, any> {
    ...
    updateHiddenFilters(value: any, idx: number) {
    ...
    componentDidUpdate(prevProps: FilterProps, prevState: any, snapshot: any) {
                ...
                {app == 'csv' && this.state.filters && <PanelBody initialOpen={false} title={__("Select Filter")}>
                    ...
                    <PanelRow>
                        <SelectControl
                            ...
                            value={defaultValueCriteria as "DEFAULT_VALUE_INPUT" | "LOWEST_VALUE" | "HIGHEST_VALUE"}
