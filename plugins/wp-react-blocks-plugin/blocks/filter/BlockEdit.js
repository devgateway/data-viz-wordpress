  ...
  if (filterType === "multi-select") {
    ...
    const options = [
      ...
      ...sortedItems.map((v) => ({
        label: v.value,
        value:
          typeof v.id === "boolean" ? (v.id ? "true" : "false") : String(v.id),
      })),
    ];
...
class BlockEdit extends BlockEditWithAPIMetadata {
  ...
  updateHiddenFilters(value, idx) {
    const {
      attributes: { hiddenFilters },
      setAttributes,
    } = this.props;
    if (hiddenFilters.indexOf(value) > -1) {
      setAttributes({
        hiddenFilters: hiddenFilters.filter((item) => item !== value),
      });
    } else {
  ...
  componentDidUpdate(prevProps, prevState, snapshot) {
    ...
    if (
      attributes &&
      attributes.filterType === "single-select" &&
      attributes.defaultTopNEnabled &&
      attributes.defaultTopNCount > 1
    ) {
  ...
  render() {
    ...
    const selectedFilters = this.state.filters
      ? this.state.filters.filter(
          (f) => f.param == param && f.type != "Boolean",
        )
      : null;
    const filterAnyType = this.state.filters
      ? this.state.filters.filter((f) => f.param == param)
      : null;

    const filter =
      selectedFilters && selectedFilters.length > 0 ? selectedFilters[0] : null;
      ...
      isSelected && (
        <InspectorControls>
          <Panel header={__("Filter Configuration")}>
            ...
            {app && app !== "csv" && (
              <PanelBody initialOpen={false} title={__("Select Filter")}>
                {this.state.filters && (
                  <>
                    <PanelRow>
                      <SelectControl
                        ...
                        value={param}
                        options={[
                          { value: "", label: __("Select Filter") },
                          ...this.state.filters,
                        ]}
                        onChange={(param) => {
                          if (param != "") {
                            const type = this.state.filters.filter(
                              (f) => f.param === param,
                            )[0].type;
                            setAttributes({
                              param,
                    ...
                    <PanelRow>
                      <SelectControl
                        ...
                        value={parentFilter}
                        help={__(
                          "Set parent filter autoaply false in order to avoid unnecesary data reloading.",
                        )}
                        options={[
                          { value: "", label: __("None") },
                          ...this.state.filters,
                        ...
                        onChange={(parentFilter) => {
                          if (parentFilter && parentFilter !== "") {
                            const parentFilterObj = this.state.filters.find(
                              (f) => f.value === parentFilter,
                            );
                            if (parentFilterObj) {
                              setAttributes({
                                parentFilter,
                                parentFilterParam: parentFilterObj.param,
                              });
                            }
                          } else {
                            setAttributes({
                ...
                <PanelRow>
                  <SelectControl
                    ...
                    options={[
                      ...
                      {
                        value: LOWEST_VALUE,
                        label: "Lowest value in filter data",
                      },
                      {
                        value: HIGHEST_VALUE,
                        label: "Highest value in filter data",
                      },
                    ]}
                    help={__("How to determine the default value.")}
                  />
                </PanelRow>
                {defaultValueCriteria === DEFAULT_VALUE_INPUT && (
                  <PanelRow>
                    <TextControl
            ...
            {app !== "csv" && (
              <PanelBody initialOpen={false} title={__("Filter or hide items")}>
                <PanelRow>
                  <ToggleControl
                ...
                {!useFilterItems && (
                  <PanelBody
                    initialOpen={false}
                    title={__("Hidden Filter Options")}
                  >
                ...
                <PanelBody initialOpen={false} title={__("Default Selection")}>
                  <PanelRow>
                    <ToggleControl
                      ...
                      help={__(
                        "Enable selecting the first N items by default.",
                      )}
                    />
                  </PanelRow>
                  {defaultTopNEnabled && (
                    <PanelRow>
                      <TextControl
                        ...
                        value={
                          filterType === "single-select" ? 1 : defaultTopNCount
                        }
                        ...
                        onChange={(val) => {
                          ...
                          if (filterType === "single-select") {
                            ...
                            setAttributes({
                              defaultTopNCount: isNaN(n) ? 0 : n,
                            });
                          }
                        }}
                        help={
                          filterType === "single-select"
                            ? __("Single-select caps Top N to 1.")
                            : __("Number of items to preselect when enabled.")
                        }
                      />
                    </PanelRow>
                  )}
            ...
            <PanelBody title={__("Dropdown Options")}>
              <PanelRow>
                <ToggleControl
                  ...
                  onChange={() =>
                    setAttributes({ alphabeticalSort: !alphabeticalSort })
                  }
              ...
              <PanelRow>
                <ToggleControl
                  ...
                  onChange={() =>
                    setAttributes({ useSingleColumn: !useSingleColumn })
                  }
              ...
              <PanelRow>
                <ToggleControl
                  label={__("Enable Text Search")}
                  checked={enableTextSearch}
                  onChange={() =>
                    setAttributes({ enableTextSearch: !enableTextSearch })
                  }
              ...
              <PanelRow>
                <ToggleControl
                  label={__("Show No Data Option (if available)")}
                  checked={showNoDataOption}
                  onChange={() =>
                    setAttributes({ showNoDataOption: !showNoDataOption })
                  }
                  help={__(
                    "Show a message when filter list is empty is empty.",
                  )}
                />
              </PanelRow>
                ...
                <>
                  <PanelRow>
                    <ToggleControl
                      ...
                      onChange={() =>
                        setAttributes({ closeOnSelect: !closeOnSelect })
                      }
                  ...
                  <PanelRow>
                    <ToggleControl
                      ...
                      help={__(
                        "Treat 'All' and 'None' selections identically.",
                      )}
                    />
                  </PanelRow>
                </>
              ...
              <PanelBody title={__("Auto Apply")}>
                <PanelRow>
                  <ToggleControl
                    ...
                    help={__(
                      "Automatically apply filter changes without a submit button.",
                    )}
                  />
                </PanelRow>
              </PanelBody>
