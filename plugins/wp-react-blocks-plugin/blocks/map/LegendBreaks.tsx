...
import { LegendBreak, LegendBreaksProps } from './types';
...
export default class LegendBreaks extends Component<LegendBreaksProps> {
    constructor(props: LegendBreaksProps) {
        super(props);
    ...
    setColor(color: string, idx: number) {
        const { attributes: { legendBreaks }, setAttributes } = this.props
        const newLegendBreak = legendBreaks.slice()
        newLegendBreak[idx].color = color ? encodeURIComponent(color) : undefined;
        setAttributes({ legendBreaks: newLegendBreak })
    }
    ...
    addLegendBreaks() {
        ...
        const newLegendBreak: LegendBreak = { min: 0, max: 0 }
    ...
    render() {
        ...
        const csvMeasureOPtions: { value: string; label: string }[] = []
        if (app == 'csv' && csv) {
        ...
        return [<PanelBody initialOpen={false} title={__("Colors")}>
            ...
            {autoGenerateBreaks &&
                <>
                    <PanelRow>
                        {<TextControl value={numberOfBreaks || 0} label={__("Number of breaks")}
                            onChange={(value) => setAttributes({ numberOfBreaks: parseInt(value) <= 10 ? parseInt(value) : numberOfBreaks })}
                            max={10}
                            type="number" />}
                    </PanelRow>
                    <PanelRow>
