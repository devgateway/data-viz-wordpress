...
import { BlockEditWithAPIMetadata, MapCSVSourceConfig, Media, Taxonomy, isSupersetAPI, APIConfig } from '@devgateway/dvz-wp-commons';
import LegendBreaks from "./LegendBreaks"
import MapSymbols from "./Symbols"
import Tooltips from "./Tooltips"
import Settings from "./Settings"
import apiFetch from '@wordpress/api-fetch';
import { MapBlockProps, MapBlockState } from './types';

class BlockEdit extends BlockEditWithAPIMetadata<MapBlockProps, MapBlockState> {
    constructor(props: MapBlockProps) {
        super(props);
        this.state = {
            taxonomyValues: [],
            types: 'media',
    ...
    componentDidUpdate(prevProps: MapBlockProps, prevState: MapBlockState, snapshot) {
    ...
    shouldComponentUpdate(nextProps: MapBlockProps, nextState: MapBlockState) {
        try {
    ...
    getTaxonomyValues(taxonomies) {
        const typesList: { slug: string, label: string, value: string }[] = []
        if (taxonomies) {
    ...
    getMapFiles() {
        ...
        apiFetch<Media[]>({
            ...
            if (json) {
                json.forEach(f => {
                    mapFiles.push({ value: f.id.toString(), label: f.title.rendered })
                })
    ...
    taxonomyValueOptions() {
        ...
        const taxonomyValuesOptions = taxonomyValues && taxonomyValues.map(t => ({ label: t.name, value: t.id.toString(), taxonomy: t.taxonomy }))

        return [{ label: 'None', value: 'none', taxonomy: 'none' }, ...taxonomyValuesOptions]
    }

    getCollectionField(data?: any) {
    ...
    mappingFieldOptions() {
        const mappingFields: { value: string, label: string }[] = []
    ...
    onFileTypeChanged(value: string) {
    ...
    updateLocationsAndCSV(layerData: any, field?: string) {
    ...
    getLayerColor(value: string, field: string) {
        const { attributes: {
            enabledLayers
        }
        } = this.props;
    ...
    setLayerColor(value: string, field: string, color: string | undefined) {
        const { setAttributes, attributes: {
            enabledLayers
        }
        } = this.props;
    ...
    setValue(id: string, field: string, value: string) {
        const { setAttributes, attributes: {
            enabledLayers
        } } = this.props;
            ...
            <Panel header={__("Map Configuration")}>
                <PanelBody initialOpen={false} title={__("Map Type")}>
                    <PanelRow>
                        <SelectControl
                            label={__('Map Type')}
                            value={mapType as 'DEFAULT' | 'POINTS_MAP'}
                            onChange={(value) => {
                    ...
                    <PanelBody initialOpen={true} title={__("Map Layers")}>
                        ...
                        <PanelRow>
                            <SelectControl
                                ...
                                value={showShadingLayerLabels as 'ifUnitHasData' | 'doNotShow' | 'showAll'}
                                onChange={(value) => {
                        ...
                        <PanelRow>
                            <SelectControl
                                label={__('Map Center')}
                                value={mapCenter as 'KEN' | 'NGA' | 'ZAF' | 'West Africa' | 'Africa' | 'ETH' | 'ZMB' | 'DRC' | 'World'}
                                onChange={(value) => {
                ...
                {/* @ts-ignore TODO: fix this */}
                <MapSymbols {...this.props} mapSymbols={[]} locations={this.state.locations || []} />
                {/* @ts-ignore TODO: fix this */}
                <Tooltips {...this.props} locations={this.state.locations || []} />
                {mapType == "POINTS_MAP" &&
                    <PanelBody title="Point Map Config">
                        ...
                        <PanelRow>
                            <SelectControl
                                ...
                                value={aggregationFormula as 'COUNT' | 'SUM'}
                                onChange={(value) => {
