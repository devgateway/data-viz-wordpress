...
class BlockEdit extends BlockEditWithAPIMetadata {
  ...
  getMapFiles() {
    const {
      attributes: { fileType, taxonomy },
    } = this.props;

    const fileTypeFilter =
      taxonomy != "none" && fileType != "none"
        ? `&${taxonomy}=${fileType}`
        : "";
    const mapFiles = [{ value: "", label: "None" }];
    wp.apiFetch({
      path:
        "/wp/v2/media?per_page=100&mime_type=application/json" + fileTypeFilter,
    }).then((json) => {
      if (json) {
        json.forEach((f) => {
  ...
  taxonomyValueOptions() {
    ...
    const taxonomyValuesOptions =
      taxonomyValues &&
      taxonomyValues.map((t) => ({
        label: t.name,
        value: t.id,
        taxonomy: t.taxonomy,
      }));
    ...
    return [
      { label: "None", value: "none", taxonomy: "none" },
      ...taxonomyValuesOptions,
    ];
  }
  ...
  onFileTypeChanged(value) {
    ...
    setAttributes({
      fileType: value,
      taxonomy: taxonomyValue ? taxonomyValue.taxonomy : "none",
    });
  }
  ...
  extractFeatures() {
    ...
    if (
      layerData &&
      layerData.objects &&
      layerData.objects[collectionName] &&
      layerData.objects[collectionName].geometries
    ) {
  ...
  getApiLabels(type, locale) {
    const labels = this.state.categories?.find(
  ...
  applyLocalization(layerId) {
    ...
    if (
      ...
      apiField === "none" ||
      !layerMappingField ||
      layerMappingField === "none"
    ) {
      alert(
        __(
          "Please select both API field and Mapping field before applying localization",
        ),
      );
      return;
    }
    ...
    if (selectedField) {
      ...
      features.forEach((g) => {
        if (g.properties && g.properties[selectedField]) {
          ...
          const label =
            apiLabels && apiLabels.has(location)
              ? apiLabels.get(location)
              : location;
      ...
      const csvFieldIsBlank =
        csv == null || csv.trim().length == 0 || csv.trim() == "name,value";
      if (csvFieldIsBlank) {
        let text = "name,value\n";
  ...
  setLayerColor(value, field, color) {
    ...
    if (enabledLayers) {
      ...
      values[index][field] = color
        ? encodeURIComponent(color)
        : encodeURIComponent("#f8f8f8");
      ...
      isSelected && (
        <InspectorControls>
          <Panel header={__("Map Configuration")}>
              ...
              <PanelRow>
                <SelectControl
                  ...
                  options={[
                    ...
                    { label: "Points Map", value: "POINTS_MAP" },
                  ]}
                />
              </PanelRow>
            </PanelBody>
              ...
              <PanelBody initialOpen={true} title={__("Map Layers")}>
                    ...
                    .map((file) => {
                      return (
                        <>
                          ...
                          {enabledLayers.findIndex((l) => l.id == file.value) >
                            -1 && (
                            <>
                              <PanelRow>
                                <PanelColorSettings
                              ...
                              <div style={{ padding: "20px" }}>
                                {
                                  <TextControl
                                    value={this.getValue(file.value, "index")}
                                    label={__("Index")}
                                    onChange={(value) => {
                                      this.setValue(file.value, "index", value);
                                    }}
                                    max={10}
                                    type="number"
                                  />
                              ...
                              {app !== "csv" && (
                                ...
                                >
                                  <PanelRow>
                                    ...
                                    >
                                      {__(
                                        "Note: Use this feature only when there is corresponding API data for this layer in the database. This will apply localized labels to map regions based on the selected API field and locale.",
                                      )}
                                    </p>
                                  </PanelRow>
                                  <PanelRow>
                                  ...
                                  <PanelRow>
                                    <SelectControl
                                      label={__("API Field")}
                                      value={[
                                        this.getValue(file.value, "apiField") ||
                                          "none",
                                      ]}
                                      onChange={(value) => {
                                        this.setValue(
                                  ...
                                  <PanelRow>
                                    <SelectControl
                                      ...
                                      value={[
                                        this.getValue(file.value, "locale") ||
                                          "en",
                                      ]}
                                      onChange={(value) => {
                                        this.setValue(
                                  ...
                                  <PanelRow>
                                    <Button
                                      variant="primary"
                                      onClick={() =>
                                        this.applyLocalization(file.value)
                                      }
                                      disabled={
                                        !(
                                          this.getValue(
                ...
                <PanelRow>
                  <SelectControl
                    ...
                    options={
                      this.state.mapFiles
                        ? this.state.mapFiles.filter(
                            (m) =>
                              !m.value ||
                              enabledLayers.findIndex((l) => l.id == m.value) >
                                -1,
                          )
                        : []
                    }
                ...
                <PanelRow>
                  <SelectControl
                    ...
                    options={[
                      {
                        label: "Only if admin unit has data",
                        value: "ifUnitHasData",
                      },
                      { label: "Do not show", value: "doNotShow" },
            ...
            {app == "csv" && (
              <MapCSVSourceConfig {...this.props}></MapCSVSourceConfig>
            )}
            <Settings {...this.props} locations={this.state.locations}>
              {" "}
            </Settings>
            <LegendBreaks
              {...this.props}
              ...
              <PanelBody title="Point Map Config">
                    ...
                    colorSettings={[
                      {
                        value: decodeURIComponent(
                          defaultPointColor ? defaultPointColor : "#f0f0f1",
                        ),
                        onChange: (color) => {
                          if (color) {
                            setAttributes({
                              defaultPointColor: encodeURIComponent(color),
                            });
                          } else {
                ...
                <PanelRow>
                  <ToggleControl
                    ...
                    onChange={() =>
                      setAttributes({ zoomOnFilter: !zoomOnFilter })
                    }
                  />
                </PanelRow>
                ...
                {zoomOnFilter && (
                  <PanelRow>
                    <TextControl
                      ...
                      onChange={(zoomOnFilterField) =>
                        setAttributes({ zoomOnFilterField })
                      }
                    />
                  </PanelRow>
                )}
