		...
		if ( null === wpm_get_post_config( $post->post_type ) && ( wpm_is_ml_string( $post->post_title ) || wpm_is_ml_value( $post->post_content ) ) ) {
			...
			<div class="misc-pub-section language">
				...
				<?php if ( $languages[ $language ]['flag'] ) { ?>
					<img src="<?php echo esc_url( wpm_get_flag_url( $languages[ $language ]['flag'] ) ); // phpcs:ignore PluginCheck.CodeAnalysis.***REMOVED***.***REMOVED*** -- Reason Using built in function doesn't work in our case, so created custom function ?>" alt="<?php echo esc_attr( $languages[ $language ]['name'] ) ; ?>">
				<?php } else { ?>
