...
class WPM_Schema_Saswp {
	...
	public function load_config( $config ) {
	    ...
	    if ( is_admin() ) {
	    	...
	    	// phpcs:ignore WordPress.Security.***REMOVED***.Recommended --Reason Not a form submission data just accessing the values for conditional check
	    	if ( ! empty( $_GET['post'] ) ){
	    		// phpcs:ignore WordPress.Security.***REMOVED***.Recommended --Reason Not a form submission data just accessing the values for conditional check
	    		...
	    		if ( $schema_id > 0 && $id > 0 ) {
	    			...
	    			if ( $is_term == 1 ) {
            			...
            			if ( false === $termmeta_value ) {
	    					// phpcs:ignore WordPress.DB.***REMOVED***.DirectQuery --Reason Using built function doesn't work in our case, so added manual query
	...
	private function set_value( $key, $value ) {
		...
		if ( empty( $current_value ) ) {
			...
			// phpcs:ignore WordPress.DB.***REMOVED***.DirectQuery, WordPress.DB.***REMOVED***.NoCaching --Reason Using built function doesn't work in our case, so added manual query
