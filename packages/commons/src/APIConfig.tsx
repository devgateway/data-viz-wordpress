...
export class APIConfig extends Component<APIConfigProps> {
  ...
  addFilter() {
    ...
    const newFilter =
      allFilters && allFilters.length > 0
        ...
        : null;
    let newFilters: Array<Filter | null> = filters.slice();
  ...
  onCustomLabelToggleChange(value: string) {
    const {
      setAttributes,
      attributes: { app, measures },
    } = this.props;
  ...
  onCustomLabelChange(value: string, customLabel: string) {
    const {
      setAttributes,
      attributes: { app, measures },
    } = this.props;
  ...
  items(type: string) {
    ...
    let items: Array<{ value: string; id: number }> | null = null;
    if (type === "Boolean") {
      items = [
        { value: "Yes", id: 1 },
        { value: "No", id: 0 },
