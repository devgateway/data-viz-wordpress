...
const getOptionsNoDimension = (props) => {
    ...
    if (***REMOVED***.length == 0 && data) {
        ...
        if (data.metadata && data.metadata.measures) {
            const ***REMOVED*** = data.metadata.measures
        ...
        if (swap && ***REMOVED***.length == 1 && measures.length > 0) {
            ...
            ***REMOVED***.forEach((measure) => {
                ...
                row["measure"] =
                    ...
                    ***REMOVED***(mMap[measure.value], locale); // measureLabel(mMap, m)
        ...
        if (props.sort == "***REMOVED***") {
            ...
            filtered.sort((a, b) => {
                if (props.***REMOVED*** == "_total") {
                    ...
                    //props.***REMOVED*** is value of category
                    ...
                    if (data?.metadata?.types?.length > 1) {
                        const translatedSor2Dimension =
                            data?.metadata?.types[1].items.filter((c) => {
                                if (
                                    props.***REMOVED*** === c.value ||
                                    (c.labels && c.labels[upperLocale] === props.***REMOVED***)
                                ) {
                                    return true;
                                }
                                return false;
