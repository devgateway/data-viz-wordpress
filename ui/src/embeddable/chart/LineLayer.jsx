...
const Line = (data, lineColor, layout, groupMode, keys, ***REMOVED***, title,measure) => injectIntl((props) => {
    ...
    if (data) {
        ...
        lineGenerator.x(data => {
            if (layout === "horizontal") {
                return xScale(data[xIndex])
            } else {
                return xScale(data[xIndex]) + (barWidth * (centerLine ? keys.length : 1)) / 2
            }            
        })
        .y(data => {
            if (layout === "horizontal") {
                return yScale(data[yIndex]) + (barWidth * (centerLine ? keys.length : 1)) / 2
            } else {
        ...
        return (
            <Fragment>
                ...
                />
                {lineData.map(d => {
                    let ***REMOVED*** = layout === "horizontal" ? ((barWidth * (centerLine ? keys.length : 1)) / 2) : 0
                    let ***REMOVED*** = layout === "horizontal" ? 0 : (barWidth * (centerLine ? keys.length : 1)) / 2
                    ...
                    return (
                        <circle
                            ...
                            onMouseMove={(event) => {
                                tooltip.html(formatContent(***REMOVED***, { x: d[0], y: d[1] ,title,measure}, intl))
                                tooltip.style("top", (event.pageY - tooltip.node().getBoundingClientRect().height) + "px").style("left", (event.pageX - tooltip.node().getBoundingClientRect().width) + "px")
                            }}
